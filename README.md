...
- IPA/BUC 字符会插入到当前光标或替换选区，编辑内容按用户和任务自动保存到本地草稿。
- PDF 预览支持缩放、适宽、旋转、全屏和页码定位，窄屏下编辑器自动切换为上下布局。
- 支持 `Ctrl/⌘+S` 保存草稿、`Ctrl/⌘+Enter` 提交、`Alt+←/→` 切换任务。
- 项目大厅按项目汇总总条目、一校待处理、二校待处理、我的进行中、完成进度和不一致次数。
- 二校任务必须由不同于一校的校对员完成，且二校编辑器不会预填一校结果。
- 一校和二校结果只写入受限的校对历史集合；比较和状态转换由后端事务完成。
- 两次校对一致自动标记为 `approved`；不一致进入 `arbitration`，由管理员逐字段确认最终值。
- 个人主页展示参与项目数、校对次数、双校一致率和排行，不一致记录不会从统计中消失。
...
2. 在右侧表格中逐字段校对。
3. 使用 IPA/BUC 键盘在当前光标处插入特殊字符，也可以替换已选中的文本。
...
- 第二次校对独立从原始 CSV 内容开始，不显示第一次校对结果。
- 第二次校对与第一次字段内容完全一致时，条目状态变为 `approved`。
- 第二次校对与第一次不一致时，`mismatch_count` 增加，条目变为 `arbitration`；双方结果会保存在 `proofreading_attempts` 中。

### 9. 管理员仲裁不一致条目

1. 在项目详情页找到“待仲裁”条目并点击“仲裁”。
2. 对照原始内容、一校结果和二校结果逐字段确认最终值。
3. 可填写仲裁说明，提交后条目变为 `approved`。

仲裁不会覆盖或删除一校、二校历史，最终值会作为第三条 `arbitration` 尝试保存。
...
同一条目的一校和二校会分别计入参与记录；尚未完成二校的尝试暂不进入一致率分母。
...
`npm test` 运行前端单元测试，覆盖光标/选区插入、本地草稿、跨任务反馈，以及管理员条目筛选、分页和范围解析。
...
- 原子认领 `pending` 或 `proofread` 条目。
- 二校不能由一校同一位校对员完成。
- 保存不可被另一位校对员读取的校对尝试。
- 在后端比较两次结构化结果并决定 `approved` 或 `arbitration`。
- 仅允许管理员读取双方结果和提交仲裁。

### 第一阶段集成测试

测试覆盖越权修改拦截、二校盲校、一致自动通过、不一致转仲裁、管理员仲裁和历史统计：
